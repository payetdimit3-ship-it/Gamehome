...
// 🤖 MULTI-AI INTEGRATION (DeepSeek, Claude, ChatGPT, Gemini)
async function askAI(prompt, preferred) {
  ...
  if (!configured.length) {
    return 'AI API key haijawekwa. Weka DEEPSEEK_API_KEY, ANTHROPIC_API_KEY, OPENAI_API_KEY au GEMINI_API_KEY kwenye Render.';
  }
...
app.post('/api/ai/health', async (req, res) => {
  ...
  const prompt = `Wewe ni msaidizi wa taarifa za afya wa jamii ndani ya GameHub. Jibu kwa Kiswahili rahisi na kwa usalama. Toa taarifa za jumla tu; usijidai kuwa daktari, usifanye diagnosis ya uhakika, na usiagize dawa kwa mtu binafsi. Kama kuna dalili kali au dharura (kupumua kwa shida, maumivu makali ya kifua, kupoteza fahamu, kutokwa damu nyingi, degedege, au hatari ya kujiumiza), mshauri mtumiaji kutafuta huduma ya dharura mara moja. Usiombe taarifa nyeti zisizo muhimu.\nMazungumzo:\n${transcript}\nSwali jipya: ${String(message).slice(0, 5000)}`;
  const reply = await askAI(prompt, provider || process.env.NEXUS_HEALTH_PROVIDER || 'google');
  res.json({ reply });
});
...
app.post('/api/admin/movies/upload', upload.single('video'), async (req, res) => {
  const user = getUserByToken(req); if (!user || !user.isAdmin) return res.status(403).json({ error: 'Wewe si admin' });
  if (!req.file && !req.body.videoUrl) return res.status(400).json({ error: 'Chagua movie/video au weka link kwanza.' });
  try {
...
async function askClaudeBuilder(command) {
  const system = `Wewe ni AI Web Developer & Manager wa GameHub. Tengeneza JSON TU, bila markdown wala maelezo mengine.
...
app.post('/api/admin/ai-builder', async (req, res) => {
  ...
  try {
    const plan = await askClaudeBuilder(command); if (!plan || !Array.isArray(plan.actions)) return res.status(422).json({ error: 'AI haikutoa JSON sahihi.', raw: plan });
    const results = [];
    ...
    for (const action of plan.actions.slice(0, 10)) {
      ...
      if (type === 'add_product') {
        ...
        products[id] = { id, name: String(action.name || 'New Product').slice(0, 160), type: String(action.type || action.section || 'Game').slice(0, 80), price: Number(action.price || 0), desc: String(action.desc || '').slice(0, 800), image: String(action.image || '').slice(0, 1000), section: String(action.section || 'shop').slice(0, 30) };
